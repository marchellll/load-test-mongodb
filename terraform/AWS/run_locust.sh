...
declare -a locustExecuteFile=$(terraform output locust_execution_file | sed -r 's/,/" "/g')
...
nohup locust --master -f "${locustExecuteFile}" >> master.log 2>&1 &
EOF
...
for host in ${workerList[@]}; do
    ...
    do
        nohup locust --worker --master-host="${master}" -f "${locustExecuteFile}" >> worker"$c".log 2>&1 &
        sleep 1
    done
EOF
done
